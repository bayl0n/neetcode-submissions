class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        we know water can be trapped if for a given height, it is less than the max_elevation
        for i and j

        - we want to track the max elevation between and i j
        - when a new max elevation is found, we want to find the water heights between i and j for heights
            and add them to our count
        - if a height taller than our max elevation is found, we stop scanning and move our pointer to it
